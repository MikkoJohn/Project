...
if(isset($_POST['convert_jo'])){
  $jo_id = $_POST['jo_id'];
  $sql_jo = "SELECT client_name, job_order_control_no, proj_name, pages, finishing_required FROM job_order WHERE job_order_control_no = '".$jo_id."'";
...
if($resulta->num_rows >= 1) {
  ...
  $stmt = $conn->prepare("INSERT INTO job_ticket (client_name,job_order_control_no,proj_name,pages,binding) VALUES (?,?,?,?,?) ");
  //$status= "Disabled";
  ...
  //$stmt = $conn->prepare("UPDATE `job_order` SET status=? WHERE `job_order_control_no` = ?");
                              ...
                              $stmt->bind_param('sssss',$client_name,$job_no,$proj_name,$no_pages,$f_required);
